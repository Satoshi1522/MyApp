      ...
      <div class="detail-wrapper">
        <div class="main-wrapper">
          <div class="h2-wrapper">
            ...
            @if ($order->status == false && $order->cancel_flag == false)
            <div>
              <a class="link-button" href="{{ route('admin.order.edit',['id' => $client->id]) }}">編集</a>
              <a class="link-button" href="{{ route('admin.order.cancel', ['id' => $client->orders->id]) }}" onclick="return orderCansel_confirm(event);">注文キャンセル</a>
            </div>
            @endif
          </div>
          ...
          <div class="client-infomation">
            <div class="name">
              <p class="bold">氏名</p>
              <div>{{ $client->lastname }}{{ $client->name }}</div>
            </div>
            <div class="name">
              <p class="bold">カナ</p>
              <div>{{ $client->lastname_furigana }}{{ $client->name_furigana }}</div>
            </div>
          </div>
          <div class="client-infomation">
            <div class="name">
          ...
          <div class="address-infomation">
            ...
            <div class="address">
              <p class="bold">住所</p>
              <div>{{ $client->prefectures }} {{ $client->town }}{{ $client->building }}</div>
            </div>
          </div>
        ...
        <div class="comment-wrapper">
          <form id="vue_form" method="post" action="{{ route('admin.order.comment.store', ['id' => $client->id]) }}">
            @csrf
